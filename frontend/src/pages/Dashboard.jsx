...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    dashboardApi.getSummary(month + 1, year)
      .then(d => setData(d))
      .catch(() => setData(generateMonthData(month + 1, year)))
      .finally(() => setLoading(false));
  }, [month, year]);

  const d = data || generateMonthData(month + 1, year);

  const metrics = [
    { label: 'Total Revenue', value: fmt(d.revenue), icon: TrendingUp, color: 'green', change: '+12.4%' },
    { label: 'Total Expenses', value: fmt(d.expenses), icon: TrendingDown, color: 'red', change: '+3.1%' },
    { label: 'Net Profit', value: fmt(d.netProfit), icon: DollarSign, color: 'blue', change: '+18.2%' },
    { label: 'Cash Balance', value: fmt(d.cashBalance), icon: Wallet, color: 'purple', change: '+5.7%' },
    { label: 'Pending Txns', value: d.pendingTransactions, icon: Clock, color: 'orange', change: '' },
    { label: 'Outstanding', value: fmt(d.outstandingPayments), icon: AlertCircle, color: 'yellow', change: '' },
  ];
  ...
  return (
    <Layout>
      ...
      <div className="period-label">
        ...
        {loading && <span style={{ marginLeft: 8, color: 'var(--text)', fontSize: '0.75rem' }}>Loading...</span>}
      </div>
      ...
      <div className="charts-row">
        ...
        <div className="chart-card chart-card--animate" style={{ animationDelay: '100ms' }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={d.expenseByCategory} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80}
